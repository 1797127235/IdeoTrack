...
created: 2026-06-22
updated: 2026-06-23
changelog: 2026-06-23 应用 sprint-change-proposal-2026-06-23，新增 AD-17 客户端按角色分端，更新 Stack 表与 Structural Seed
binds:
  - FR-1..FR-31
  - UJ-1..UJ-3
  - SM-1..SM-C2
...
- **Rule:** Clients are deployed per role:
  ...
  - **Counselor + Admin client** → React Native + Expo App (located at `mobile/`, reuses existing code, role-routed into different views)

  Deployment boundaries follow these constraints:
  - Students log in via WeChat login (`wx.login` + first-time student-ID binding); counselors log in via staff-ID + password; admins via account + password (all reuse the existing JWT system, AD-4)
  - All file download/export (FR-24 counselor export, FR-28 report export) runs in the Expo App only, never in the Mini Program (WeChat file sandbox limitation)
  - Student notifications use WeChat subscribe messages; counselor/admin notifications use in-App notifications
  - The backend API is fully platform-agnostic; both ends share the same REST interface
...
| Name | Version / Note |
| --- | --- |
| React Native | 0.85 (via Expo SDK 56) — counselor + admin client only |
| 微信小程序原生 (WeChat Mini Program) | base library 3.x — student client |
| Expo | 56.x |
...
```text
ideo-track/
├── mobile/                         # React Native + Expo — counselor + admin client (AD-17)
│   ├── app/
│   │   ├── navigation/
│   │   ├── screens/
...
│   └── package.json
├── miniprogram/                    # WeChat Mini Program — student client only (AD-17)
│   ├── pages/
│   │   ├── auth/                   # WeChat login + student-ID binding
...
| --- | --- | --- |
| 学生登录/角色鉴权 | `api/src/domains/auth` + `mobile/app/services` | AD-4, AD-5 |
| 任务发布与展示 | `api/src/domains/tasks` + `mobile/app/screens` | AD-1, AD-10 |
| 定位签到与心得提交 | `api/src/domains/checkins` | AD-1, AD-6, AD-10 |
| AI 初审 | `api/src/adapters/llm` + `api/src/domains/reviews` | AD-3, AD-6 |
| 辅导员人工复核 | `api/src/domains/reviews` + `mobile/counselor` | AD-5, AD-10 |

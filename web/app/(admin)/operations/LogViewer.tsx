          ...
          <div className="overflow-auto max-h-[60vh]">
            <table className="w-full text-sm">
              <thead className="sticky top-0 z-10 bg-[var(--color-surface)] border-b border-[var(--color-border)]">
                <tr>
                  <th className="text-left py-2.5 px-4 text-xs font-semibold uppercase tracking-wider text-[var(--color-ink-muted)] whitespace-nowrap">
                    时间
                  </th>
                  <th className="text-left py-2.5 px-3 text-xs font-semibold uppercase tracking-wider text-[var(--color-ink-muted)] whitespace-nowrap">
                    级别
                  </th>
                  <th className="text-left py-2.5 px-3 text-xs font-semibold uppercase tracking-wider text-[var(--color-ink-muted)] whitespace-nowrap">
                    方法
                  ...
                  <th className="py-2.5 px-4 text-right text-xs font-semibold uppercase tracking-wider text-[var(--color-ink-muted)] whitespace-nowrap">
                    详情
                  </th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((entry, idx) => (
                  <>
                    ...
                    >
                      <td className="py-2.5 px-4 text-xs text-[var(--color-ink-secondary)] whitespace-nowrap">
                        {formatLogTime(entry.time)}
                      </td>
                      <td className="py-2.5 px-3 whitespace-nowrap">
                        {entry.parsed && entry.level !== undefined ? (
                          <Badge variant={levelVariant(entry.level)}>{levelName(entry.level)}</Badge>
                        ) : (
                          <Badge variant="neutral">-</Badge>
                        )}
                      </td>
                      <td className="py-2.5 px-3 text-[var(--color-ink)] whitespace-nowrap">
                        {entry.method || "-"}
                      </td>
                      <td className="py-2.5 px-3 text-[var(--color-ink-secondary)] whitespace-nowrap max-w-[16rem] truncate" title={entry.path}>
                        {entry.path || "-"}
                      </td>
                      <td className="py-2.5 px-3 whitespace-nowrap">
                      ...
                      <td className="py-2.5 px-4 text-[var(--color-ink)] max-w-[24rem]">
                      ...
                      <td className="py-2.5 px-4 text-right whitespace-nowrap">
                        <button
                          ...
                          className="inline-flex items-center justify-center w-7 h-7 rounded-md text-[var(--color-ink-muted)] hover:text-[var(--color-accent)] hover:bg-[var(--color-bg)] transition-colors"
                          title="查看原始 JSON"
                        >
                          {expanded[idx] ? (
                      ...
                      <tr className="bg-[var(--color-bg)] border-b border-[var(--color-border)]">
                        <td colSpan={8} className="py-3 px-4">
                          <div className="flex items-start gap-2">
                            <FileJson className="w-4 h-4 text-[var(--color-accent)] mt-0.5 shrink-0" />
                            <pre className="text-xs font-mono text-[var(--color-ink-secondary)] whitespace-pre-wrap break-all max-h-48 overflow-auto">
                              {entry.raw}
                            </pre>
                          </div>
                          {entry.parsed && (

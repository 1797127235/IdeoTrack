...
├── web/              # 管理员端 Web 后台（Next.js + App Router）
├── miniprogram/      # 学生 + 辅导员端微信小程序（原生开发，V1）
├── miniprogram-v2/   # 学生 + 辅导员端微信小程序 V2（当前主版本）
├── mobile/           # [已弃用] 前管理员端 App（React Native + Expo），仅作参考
├── test/             # API 测试集（Bruno）
├── face-service/     # 人脸识别 FastAPI 微服务
├── docs/             # 项目文档
...
├── start.bat / start.ps1  # Windows 本地开发一键启动脚本
...
详见 **[test/README.md](./test/README.md)** —— 用 Bruno 打开 `test/` 目录，覆盖认证、名言、任务、上传等业务域（以实际 collection 为准）。
...
**学生 + 辅导员端微信小程序**：
- 用微信开发者工具打开 `miniprogram-v2/` 目录（当前主版本）
- 学生通过微信登录，辅导员通过工号密码登录
- V1 代码保留在 `miniprogram/` 目录，仅作参考
- 详见 `miniprogram-v2/` 目录
...
- **任务模板库**：管理员在 Web 后台维护模板（`task_templates`），保存标题、说明、正文、封面图、分类标签、思考题、外部链接/视频/附件、打卡类型与必填项、签到/人脸要求、起止时间、状态等快照，不指定发布范围。
...
- 任务模板系统说明：[docs/TASK-TEMPLATES.md](./docs/TASK-TEMPLATES.md)
- 文件上传与存储说明：[docs/UPLOAD-AND-STORAGE.md](./docs/UPLOAD-AND-STORAGE.md)
- API 接口参考：[docs/API-REFERENCE.md](./docs/API-REFERENCE.md)
- 后端 API 部署说明：[api/README.md](./api/README.md)
- API 测试说明：[test/README.md](./test/README.md)
- 人脸服务测试部署：[docs/FACE-TEST-DEPLOY.md](./docs/FACE-TEST-DEPLOY.md)
